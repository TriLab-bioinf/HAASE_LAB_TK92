...
for (i in names(DE_results)){
    ...
    assign(gsub("Sample_","",paste0(i,"_up_Cyt_bound")),fun_enrich(sel,gsub("Sample_","",paste0(i,"_up_Nuc_bound"))))
...
p1 <- ggplot(down.enrich, aes(x= class, y=Term, size=log10(Odds.Ratio), color=Adjusted.P.value)) + 
  geom_point() + 
...
p2 <- ggplot(up.enrich, aes(x= class, y=Term, size=log10(Odds.Ratio), color=Adjusted.P.value)) + 
  geom_point() + 
  scale_color_gradient(low = "red2",  high = "yellow", space = "Lab", limit = c(0, 0.05)) +
  scale_size(range = c(1, 8))+
  theme_bw() +
...
ggsave(paste0("Plots/","up_FuncEnrich_barplot.pdf"),p2,width=10,height=8)
